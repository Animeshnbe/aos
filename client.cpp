    ...
    do{
        ...
        for(int i=0; i<4; i++){
            ...
            for(unsigned int j=0; j<chunks.size(); j++){
                if (chunks[j].havers.size()==0){
                    cout<<"Cannot download corrupted file, chunk "<<j<<" missing"<<endl;
                    corrupt=true;
                    break;
                }
                // cout<<"CMOP "<<new_file.chunkmap[j]<<endl;
                if((chunks[j].havers.size() < minval) && (new_file->chunkmap[j]=='0')){
                    minval = chunks[j].havers.size();
                    rarest = j;
        ...
        for (unsigned int i=0; i<next_block.size(); i++){
            ...
            new_file->chunkmap[next_block[i]] = '1'; //maybe not reqd.
